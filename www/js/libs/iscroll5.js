...
// INSERT POINT: OPTIONS 
		...
		useTransition: true,
		useTransform: true,
		bottomOffset: 0, //knightka 21Aug2013 added bottomOffset
	};
...
// INSERT POINT: _init
	...
	_start: function (e) {
		...
		//knightka changed from this.enabled = as value was changing and i couldn't figure out where/why
		if ( BGS.Utilities.scrollViewEnabled === false || (this.initiated && utils.eventType[e.type] !== this.initiated) ) {
			return;
		}
	...
	_move: function (e) {
		//knightka changed from this.enabled = as value was changing and i couldn't figure out where/why
		if ( BGS.Utilities.scrollViewEnabled === false || utils.eventType[e.type] !== this.initiated ) {
			return;
		}
...
/* REPLACE END: _move */
		...
		this._execEvent('scrollMove');//knightka added so we broadcast event; was left out for some reason
	...
	disable: function () {
		BGS.Utilities.scrollViewEnabled = false;//knightka changed from this.enabled = as value was changing and i couldn't figure out where/why
	},

	enable: function () {
		BGS.Utilities.scrollViewEnabled = true;//knightka changed from this.enabled = as value was changing and i couldn't figure out where/why
	},
...
/* REPLACE END: refresh */
		...
		this.wrapperOffset = utils.offset(this.wrapper);//knightka move this here

		//knightka 25Jul2013 added to fix the issue when there are less results than fill the view
		//knightka 21Aug2013 added bottomOffset property which i'll include in initial options
		this.maxScrollY >= 0 ? this.maxScrollY = (this.wrapperOffset.top - this.options.bottomOffset) : ''; //knightka 
		...
		// this.wrapperOffset = utils.offset(this.wrapper); //knightka moved this up higher to 639

		this._execEvent('refresh');

		//knightka 25Jul2013 added to fix the issue when there are less results than fill the view
		this.maxScrollY >= 0 ? this.maxScrollY = this.wrapperOffset["top"] : ''; //knightka 
...
// INSERT POINT: _translate
	...
	_wheel: function (e) {
		//knightka changed from this.enabled = as value was changing and i couldn't figure out where/why
		if ( BGS.Utilities.scrollViewEnabled === false ) {
			return;
		}
