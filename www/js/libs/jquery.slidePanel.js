    ...
    $.fn.slidePanel = function(opts) {
		...
		trigger.attr( "href", "javascript:void(0)" ).mousedown(function() {
			...
			if (opts.ajax && opts.ajaxSource!=null) {
				var src = 'templates/' + opts.ajaxSource; //knightka
				...
				if (!panel.is(':visible')) {
					...
					panel.load(src, function(response, status, xhr) {
						// if the ajax source wasn't loaded properly
						if (status !== "success") {
							var msg = "<p>Sorry, but there was an error loading the help view.</p>";
							panel.html(msg);
						};
						// this is part of the .load() callback so it fills the panel BEFORE opening it
						setTimeout(function(){panel.toggle(opts.speed);}, 200);//knightka setTimeout()
					});
				} else {
					panel.toggle(opts.speed);
				};
				trigger.toggleClass("active");
					...
					console.log('You must define an ajaxSource to use Ajax.');
			};
			return false;
		});
